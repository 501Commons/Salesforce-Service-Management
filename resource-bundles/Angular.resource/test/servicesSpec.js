...
describe('services', function() {
  ...
  beforeEach(module('ServiceManagementApp'));
  ...
  describe('fbHouseholdSearch', function() {
    ...
    it('should search households', function() {
      var resultData;
      svc.get('Boyce').then( function(result) {
        resultData = result;
      });
      /* wait for timeout so promise gets resolved */
      jasmine.clock().tick(1);
      ...
      expect(resultData[0].name).toBe('Dave Boyce');
    });
  });
  ...
  describe('fbHouseholdDetail', function() {
    ...
    it('should return a household', function() {
      ...
      expect(resultData.members[0].name).toBe('Dave Boyce');
    });
  });
